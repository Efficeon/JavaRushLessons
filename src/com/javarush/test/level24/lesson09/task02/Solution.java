    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            public int compare(Stock stock1, Stock stock2) {
                ...
                int comparName = name1.compareTo(name2);

                if (comparName!=0) return comparName;
                else {
                    SimpleDateFormat DatForm = new SimpleDateFormat("yyyyMMdd");
                    Date date1 = (Date)stock1.get("date");
                    Date date2 = (Date)stock2.get("date");

                    int comparDate = DatForm.format(date1).compareTo(DatForm.format(date2));

                    if(comparDate !=0) return comparDate;
                    else {
                        Double open;
                        Double last;
                        Double profit1;
